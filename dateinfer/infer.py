...
RULES = [
   ...
   #modified to support mm-dd-yy (preference to month over DayOfMonth: current pip version gives dd-mm-mm (12-12-12)
    ...
    If(And(Contains(Second), Sequence(F('-'), Year4)),
       SwapSequence([F('-'), Year4], [UTCOffset, None])),
    If(Sequence(F('+'), Hour24), SwapSequence([F('+'), Hour24], [UTCOffset, None])),
    If(And(Contains(Second), Sequence(F('-'), Hour24)),
       SwapSequence([F('-'), Hour24], [UTCOffset, None])),
    If(Sequence(F('+'), Minute), SwapSequence([F('+'), Minute], [UTCOffset, None])),
    If(And(Contains(Second), Sequence(F('-'), Minute)),
       SwapSequence([F('-'), Minute], [UTCOffset, None])),
    If(Sequence(F('+'), Second), SwapSequence([F('+'), Second], [UTCOffset, None])),
    If(And(Contains(Second), Sequence(F('-'), Second)),
       SwapSequence([F('-'), Second], [UTCOffset, None])),
    If(And(
